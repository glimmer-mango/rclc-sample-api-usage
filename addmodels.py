...
def main():
    ...
    print("Please wait until add model training session with job id", job_id, "has finished...")

    # wait until transfer add model training is complete
    print("Continuously polling to get add model training status...")
    pp = pprint.PrettyPrinter(indent=4)
    while True:
        r = requests.get(f"{API_URL}/jobs/{job_id}", headers=headers)
        if (r.json()['status'] == 'finished'):
            print("Add model training is complete! Check your dashboard to get the new model id.")
            break
